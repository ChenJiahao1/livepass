...
- 基于 etcd `Session + Mutex` 获取分布式锁，并使用 `Mutex.TryLock` 语义，覆盖多实例部署场景。
...
分布式锁的“立即拒绝”语义依赖 `Mutex.TryLock(ctx)`，而不是 `Lock(ctx)`。`TryLock` 发生网络往返时仍然需要一次 etcd RPC，因此这里的“立即”定义为“不等待前一个持锁请求释放”，而不是“零等待时间”。
...
`LockAcquireTimeout` 表示单次调用 etcd `TryLock` 的 RPC 截止时间，用于限制基础设施交互时长，不表示等待其他请求释放锁的时间。建议初始默认 `200ms`。
...
`program-rpc` 的 keyed mutex 容器采用“引用计数 + 解锁后回收”，避免 key 集合随节目和票档维度无限增长，同时减少后台清理线程的额外复杂度。
...
- `order-api` 对外 HTTP：`429 Too Many Requests`
- `gateway-api`：透传 `order-api` 的 HTTP 状态码，不单独重写
- 错误文案：`提交频繁，请稍后重试`
...
- etcd `Mutex.TryLock` 返回 `concurrency.ErrLocked`：视为重复提交，返回 `codes.ResourceExhausted`
...
为保证 `429/503` 稳定可见，本次交付范围补充包含 `order-api` 的 gRPC -> HTTP 状态码映射调整：

- `codes.ResourceExhausted` -> HTTP 429
- `codes.Unavailable` -> HTTP 503

`gateway-api` 不新增状态码转换逻辑，只透传 `order-api` 的 HTTP 响应。
...
- etcd `Mutex.TryLock` 使用 `LockAcquireTimeout` 控制 RPC 截止时间，不会变成短时间阻塞等待前一个请求释放锁。
...
- `CreateOrder` 的防重复路径在 etcd 交互上受 `LockAcquireTimeout` 约束，不会退化成等待前序请求释放锁后再继续建单。
