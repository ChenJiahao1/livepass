...
CREATE TABLE `d_delay_task_outbox` (
  ...
  `task_status` tinyint NOT NULL DEFAULT 0 COMMENT '0 pending, 1 published, 3 processed, 4 failed',
  `publish_attempts` int NOT NULL DEFAULT 0 COMMENT 'publish attempts',
  `consume_attempts` int NOT NULL DEFAULT 0 COMMENT 'consume attempts',
  `last_publish_error` varchar(512) NOT NULL DEFAULT '' COMMENT 'last publish error',
  `last_consume_error` varchar(512) NOT NULL DEFAULT '' COMMENT 'last consume error',
  `published_time` datetime DEFAULT NULL COMMENT 'published at',
  `processed_time` datetime DEFAULT NULL COMMENT 'processed at',
