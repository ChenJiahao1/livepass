DROP TABLE IF EXISTS `d_order_outbox_00`;
CREATE TABLE `d_order_outbox_00` (
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='订单 outbox 分片 00';

DROP TABLE IF EXISTS `d_order_outbox_01`;
CREATE TABLE `d_order_outbox_01` (
  `id` bigint NOT NULL COMMENT 'primary key',
  `order_number` bigint NOT NULL COMMENT 'order number',
  `event_type` varchar(64) NOT NULL COMMENT 'event type',
  `payload` longtext NOT NULL COMMENT 'event payload',
  ...
  KEY `idx_publish_scan` (`published_status`,`status`,`id`),
  KEY `idx_order_number` (`order_number`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='订单 outbox 分片 01';
