...
from app.agents.tools import HITL_TOOL_POLICIES, build_human_input_tool, wrap_tool_with_hitl
...
@pytest.mark.anyio
async def test_refund_execution_calls_wrapped_refund_order_directly_and_interrupts(monkeypatch):
    captured: dict[str, object] = {}
    ...
    async def _preview_refund_order(order_id: str, reason: str | None = None, user_id: int | None = None):
    ...
    async def _refund_order(order_id: str, reason: str, user_id: int | None = None):
        refund_calls.append({"order_id": order_id, "reason": reason})
        return {"order_id": order_id, "accepted": True, "refund_amount": "100"}

    raw_order_tools = [
        build_async_tool(name="list_user_orders", description="list user orders", coroutine=_list_user_orders),
        build_async_tool(
            name="get_order_detail_for_service",
            description="get order detail",
            coroutine=_get_order_detail_for_service,
        ),
        build_async_tool(name="preview_refund_order", description="preview refund", coroutine=_preview_refund_order),
    ]
    raw_refund_tool = build_async_tool(name="refund_order", description="submit refund", coroutine=_refund_order)
    registry = StubRegistry(tools_by_toolset={"order": [*raw_order_tools], "refund": [raw_refund_tool]})
    wrapped_refund_tool = wrap_tool_with_hitl(
        tool=raw_refund_tool,
        policy=HITL_TOOL_POLICIES["refund_order"],
        invoke_tool=lambda tool_name, payload: registry.invoke(
            server_name="order",
            tool_name=tool_name,
            payload=payload,
        ),
    )
    registry.tools_by_toolset["order"] = [*raw_order_tools, wrapped_refund_tool, build_human_input_tool()]
    ...
    class _FakeCreatedAgent:
        ...
        async def ainvoke(self, payload):
            ...
            captured["called_tool"] = "refund_order"
            result = await tools_by_name["refund_order"].ainvoke({"order_id": "ORD-1", "reason": "用户明确退款"})
            return {"messages": [AIMessage(content=str(result))]}

    def _fake_create_agent(*, model, tools, system_prompt, name):
        del model, system_prompt, name
        captured["tool_names"] = [tool.name for tool in tools]
        return _FakeCreatedAgent(tools)
    ...
    result = await app.ainvoke(
        {"messages": [{"role": "user", "content": "我要退款 ORD-1"}]},
        config={"configurable": {"thread_id": "conv-refund-wrapper"}},
        context={"llm": llm, "registry": registry, "current_user_id": 1001},
    )

    assert captured["called_tool"] == "refund_order"
    assert captured["tool_names"] == [
        "list_user_orders",
        "get_order_detail_for_service",
        "preview_refund_order",
    ...
    assert preview_calls == [{"order_id": "ORD-1", "reason": "用户明确退款"}]
    assert refund_calls == []
    assert result["__interrupt__"][0].value["toolName"] == "human_approval"
