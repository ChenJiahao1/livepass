...
`agents` 是 `damai-go` 的 Python 智能客服组件，当前基于 `FastAPI + MCP + Redis` 运行，父层使用轻量 `TaskCard` 编排，首期只覆盖 `order / refund / handoff` 主链路。
...
未设置 `OPENAI_API_KEY` 时，`/agent/chat` 仍可运行，会走 rule-based `ParentAgent -> TaskCard -> Runtime -> ToolBroker` fallback。
...
- 旧的 LangGraph `graph/coordinator/supervisor` 兼容层和对应 prompt 已移除；当前统一走 `ParentAgent -> TaskCard -> SubagentRuntime`。
...
- “帮我退最近那单” 会走 `order.list_recent -> refund.preview` 链路；业务失败时回退到 `handoff.create_ticket`。
...
uv run pytest tests/test_parent_agent.py tests/test_policy_engine.py tests/test_skill_resolver.py tests/test_subagent_runtime.py tests/test_no_llm_fallback.py -v
uv run pytest tests/test_agents.py tests/test_order_refund_flow.py tests/test_handoff_flow.py tests/test_session_store.py tests/test_api.py tests/test_e2e_contract.py tests/test_docs.py -v
```
