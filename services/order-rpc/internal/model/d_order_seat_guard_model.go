...
func (m *customDOrderSeatGuardModel) InsertBatch(ctx context.Context, session sqlx.Session, rows []*DOrderSeatGuard) error {
	...
	for _, row := range rows {
		...
		args = append(args, row.Id, row.OrderNumber, row.ProgramId, row.SeatId, row.CreateTime, row.EditTime, row.Status)
	}

	query := fmt.Sprintf(
		"insert into %s (`id`, `order_number`, `program_id`, `seat_id`, `create_time`, `edit_time`, `status`) values %s",
		m.table,
