...
func TestPayOrder(t *testing.T) {
	t.Run("pay success updates order and ticket snapshots to paid", func(t *testing.T) {
		...
		l := NewPayOrderLogic(context.Background(), svcCtx)
		...
		getResp, err := NewGetOrderLogic(context.Background(), svcCtx).GetOrder(&pb.GetOrderReq{
			UserId:      3001,
			OrderNumber: 92001,
		})
		if err != nil {
		...
		listResp, err := NewListOrdersLogic(context.Background(), svcCtx).ListOrders(&pb.ListOrdersReq{
			UserId:     3001,
			PageNumber: 1,
			PageSize:   10,
		})
