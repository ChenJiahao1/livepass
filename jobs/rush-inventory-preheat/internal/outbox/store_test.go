...
func TestListDispatchableByTaskTypeScansEveryShardWithoutLimit(t *testing.T) {
	...
	firstConn := &listDispatchableConn{
		rows: []delayTaskOutboxRow{{
			ID:         101,
			TaskType:   "rush_inventory.preheat",
			TaskKey:    "rush_inventory.preheat:101",
			Payload:    `{"programId":101}`,
			ExecuteAt:  executeAt,
	...
	secondConn := &listDispatchableConn{
		rows: []delayTaskOutboxRow{{
			ID:         201,
			TaskType:   "rush_inventory.preheat",
			TaskKey:    "rush_inventory.preheat:201",
			Payload:    `{"programId":201}`,
			ExecuteAt:  executeAt,
			TaskStatus: delaytask.OutboxTaskStatusPending,
		}},
	}
	...
	store := NewMysqlStore(map[string]sqlx.SqlConn{
		"program-db-0": firstConn,
		"program-db-1": secondConn,
	})
	tasks, err := store.ListDispatchableByTaskType(context.Background(), "rush_inventory.preheat")
	if err != nil {
...
func assertListQueryHasNoLimit(t *testing.T, conn *listDispatchableConn) {
	...
	if conn.args[0] != "rush_inventory.preheat" {
		t.Fatalf("task type arg = %v, want rush_inventory.preheat", conn.args[0])
	}
}
