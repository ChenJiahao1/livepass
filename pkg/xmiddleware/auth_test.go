...
func TestAuthenticateReturnsUserIDFromBearerToken(t *testing.T) {
	...
	userID, err := Authenticate(req, "X-Channel-Code", map[string]string{"0001": "secret-0001"})
	if err != nil {
...
func TestAuthenticateRejectsMissingBearerToken(t *testing.T) {
	...
	_, err := Authenticate(req, "X-Channel-Code", map[string]string{"0001": "secret-0001"})
...
func TestWithUserIDRoundTrip(t *testing.T) {
	ctx := WithUserID(context.Background(), 3001)
